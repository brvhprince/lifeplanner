...
export default function makeLoginUser({
	...
	return async function loginUser({ email, password, source }: LoginUser) {
        ...
        if (!email) {
            throw new Validation.PropertyRequiredError("Email address is required", "email");
        }

        if (!password) {
            throw new Validation.PropertyRequiredError("Password is required", "password")
        }
        ...
        if (password.length < 8) {
            throw new Validation.ValidationError("Password should be atleast 8 characters");
        }
        ...
        if (!status) {
            throw new Validation.ResponseError("Invalid credentials. Check and retry")
        }
        ...
        const { item } = await plannerDb.findUserById({ userId: user_id, details: true, profile: true})
